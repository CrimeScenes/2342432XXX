...
local function UpdateFOV()
    ...
    local success, errorMsg = pcall(function()
        if Circle then
            Circle.Visible = getgenv().Forbidden.PointLock.ProximityLimit_Visibility
            Circle.Radius = getgenv().Forbidden.PointLock.ProximityLimit  -- Corrected this line
...
Mouse.KeyDown:Connect(function(Key)
    if Key:lower() == getgenv().Forbidden.Target.Keybind:lower() then
        if getgenv().Forbidden.PointLock.Enabled then
            if IsTargeting then
    ...
    elseif Key:lower() == getgenv().Forbidden.Target.UntargetKeybind:lower() then
        IsTargeting = false
        TargetPlayer = nil  
    end
end)
...
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        ...
        local BodyPart
        if getgenv().Forbidden.PointLock.ClosestPart then
        ...
        if BodyPart then
            ...
            if getgenv().Forbidden.PointLock.Aligner then
                local humanoid = TargetPlayer.Character:FindFirstChildOfClass("Humanoid")
                if humanoid then
                    local moveDirection = humanoid.MoveDirection
                    predictedPosition = BodyPart.Position + (moveDirection * getgenv().Forbidden.PointLock.VelocityCompensation)
                end
            else
                local targetVelocity = TargetPlayer.Character.HumanoidRootPart.Velocity
                predictedPosition = BodyPart.Position + (targetVelocity * getgenv().Forbidden.PointLock.VelocityCompensation)
            end
            
            if predictedPosition then
                local DesiredCFrame = CFrame.new(Camera.CFrame.Position, predictedPosition)

                Camera.CFrame = Camera.CFrame:Lerp(DesiredCFrame, getgenv().Forbidden.PointLock.Stabilization)
            end


            if getgenv().Forbidden.SilentTargeting.Enabled and IsTargeting and TargetPlayer.Character:FindFirstChild("Humanoid") then
                if getgenv().Forbidden.SilentTargeting.mode == "target" then
                    local closestPoint
                    if getgenv().Forbidden.SilentTargeting.TargetMode == "OptimalTargetPoint" then
                        closestPoint = GetOptimalTargetPoint(TargetPlayer.Character)
                    elseif getgenv().Forbidden.SilentTargeting.TargetMode == "Closest Point" then
                        closestPoint = GetClosestPoint(TargetPlayer.Character)
                    elseif getgenv().Forbidden.SilentTargeting.TargetMode == "BasicTargeting" then
